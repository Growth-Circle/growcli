...
//! interactive-session layers. Direct `growcli login` intentionally does less: it preserves the
...
/// Installs a small file-backed tracing layer for direct `growcli login` flows.
///
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    // Direct `growcli login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `growcli login --device-auth` instead."
    );
}
...
/// `growcli login` working in environments where device-code may be disabled/feature-gated.
