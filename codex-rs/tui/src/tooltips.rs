...
const ANNOUNCEMENT_TIP_URL: Option<&str> = None;

const IS_MACOS: bool = cfg!(target_os = "macos");
const IS_WINDOWS: bool = cfg!(target_os = "windows");

const APP_TOOLTIP: &str = "Use **/status** to see the current model, approvals, and token usage.";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str = "*New* Use **/model** to switch models or reasoning effort.";
const OTHER_TOOLTIP_NON_MAC: &str = OTHER_TOOLTIP;
const FREE_GO_TOOLTIP: &str =
    "Use **/permissions** to control when Grow CLI asks for confirmation.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("growcli app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting Grow CLI.
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // Grow CLI currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Grow CLI TUI.
...
[[announcements]]
content = "Welcome to Grow CLI! Check out the new onboarding flow."
